---
title: ESLint
description: Learn when Ultracite is the right fit for ESLint, Prettier, and Stylelint, and what you keep from the mature plugin ecosystem.
---
...
In mixed-framework monorepos, the shared `ultracite.config.mjs` format scopes ESLint preset entries with flat-config `files` patterns. The generated adapter config is refreshed by `ultracite check` and `ultracite fix`; see [Monorepos](/docs/monorepos) for centralized and workspace-local examples.
...
- **tanstack** — [`@tanstack/eslint-plugin-query`](https://tanstack.com/query/latest/docs/eslint/eslint-plugin-query), [`@tanstack/eslint-plugin-router`](https://tanstack.com/router/latest/docs/eslint/eslint-plugin-router), and [`@tanstack/eslint-plugin-start`](https://tanstack.com/start/latest/docs/eslint) when you opt into the **`tanstack`** framework preset
