...
Use `ultracite.config.ts` when framework presets should apply only to part of a repository. Import `defineConfig` from `ultracite/config` for type checking; Ultracite translates the shared preset DSL into the selected provider's native config before each root `check` or `fix` run.
