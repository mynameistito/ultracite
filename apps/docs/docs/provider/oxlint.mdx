...
In mixed-framework monorepos, the shared `ultracite.config.mjs` format translates presets into Oxlint native scoped overrides. JS plugins and settings are hoisted to the root config as required by Oxlint, while framework rules stay limited to their matching files. See [Monorepos](/docs/monorepos) for both config placement styles.
