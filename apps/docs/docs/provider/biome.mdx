...
For mixed-framework repositories, use the shared `ultracite.config.ts` path-scoped format documented in [Monorepos](/docs/monorepos). Ultracite resolves the Biome preset files and writes their rule/settings payloads into Biome-native `overrides` before `check` and `fix`; Biome itself does not load the TypeScript DSL.
