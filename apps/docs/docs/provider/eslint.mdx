...
In mixed-framework monorepos, the shared `ultracite.config.ts` format scopes ESLint preset entries with flat-config `files` patterns. The generated adapter config is refreshed by `ultracite check` and `ultracite fix`; see [Monorepos](/docs/monorepos) for centralized and workspace-local examples.
