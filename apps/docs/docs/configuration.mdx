...
Use `ultracite.config.mjs` when framework presets should apply only to part of a repository. Import `defineConfig` from `ultracite/config`; it provides types for the shared preset DSL while Ultracite translates it into the selected provider's native config before each root `check` or `fix` run.
...
Files outside the override paths receive only the root presets. Workspace configs are scoped to their containing directory and inherit only configs they explicitly list in `extends`; parent layers resolve before child layers, and sibling workspaces are isolated. Both `check` and `fix` refresh generated configs in `node_modules/.cache/ultracite`, leaving the source DSL and existing provider config files untouched. Repositories without this file continue to use the provider-native config path.

`files` accepts project-relative glob patterns using `*`, `?`, `**`, character classes, and comma-separated brace alternatives (for example, `apps/{web,docs}/src/**/*.{ts,tsx}`). Parent-directory (`..`) and absolute patterns are rejected.
