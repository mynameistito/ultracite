---
title: Biome
description: See how Ultracite works with Biome's all-in-one formatter and linter, including setup, defaults, tradeoffs, and migration guidance.
---
...
For mixed-framework repositories, use the shared `ultracite.config.mjs` path-scoped format documented in [Monorepos](/docs/monorepos). Ultracite resolves the Biome preset files and writes their rule/settings payloads into Biome-native `overrides` before `check` and `fix`; Biome itself does not load the JavaScript DSL.
...
For type-aware, project-graph rules (`noPrivateImports`, `noUndeclaredDependencies`, `noUnresolvedImports`, `noImportCycles`, `noDeprecatedImports`), add the optional `type-aware` preset — or pass `--type-aware` to `npx ultracite init`:
